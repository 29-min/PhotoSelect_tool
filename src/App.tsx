...
function App() {
  ...
  const handleExport = async () => {
    ...
    if (filesToExport.length === 0) {
      alert("추출할 파일이 없습니다. (조건에 맞는 선택된 사진 없음)")
      return
    }
    ...
    alert(`총 ${copiedCount}개의 파일이 성공적으로 복사되었습니다!`)
      ...
      <header className="h-16 bg-white border-b border-zinc-200 flex items-center justify-between px-6 shrink-0 z-10">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <span className="text-sm font-bold truncate max-w-[200px]" title={folder}>{folder.split('/').pop() || folder}</span>
            <span className="text-xs text-zinc-500 font-medium">
              {visibleTotal}개 묶음 중 {visiblePosition}번째{showSelectedOnly ? ' · 선택만 보기' : ''}
            </span>
          </div>
        </div>
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        ...
        <main className="flex-1 relative bg-zinc-100 flex items-center justify-center overflow-hidden p-6">
          ...
          {currentPreview ? (
            ...
            <div className="text-zinc-400 flex flex-col items-center">
              ...
              <p className="font-medium">미리보기를 불러올 수 없습니다</p>
            </div>
          )}
        ...
        <aside className="w-60 bg-white border-l border-zinc-200 shrink-0 flex flex-col">
          <div className="h-14 px-4 border-b border-zinc-200 flex items-center justify-between">
            <div>
              <div className="text-sm font-extrabold text-zinc-900">선택된 사진</div>
              <div className="text-xs text-zinc-500">{selectedCount}개</div>
            </div>
          </div>
      ...
      {showExportModal && (
        <div className="fixed inset-0 bg-zinc-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-8">
              <h2 className="text-2xl font-extrabold mb-2 tracking-tight">사진 내보내기</h2>
              <p className="text-zinc-500 text-sm mb-6">총 {selectedCount}개의 사진 묶음이 선택되었습니다.</p>
              ...
              <div className="space-y-3 mb-8">
                <label className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${exportMode === 'both' ? 'border-black bg-zinc-50' : 'border-zinc-200 hover:border-zinc-300'}`}>
                  ...
                  <div>
                    <div className="font-bold text-zinc-900">모두 복사 (RAW + JPG)</div>
                    <div className="text-xs text-zinc-500 mt-1 leading-relaxed">선택된 사진의 원본과 JPG 파일이 모두 복사됩니다.</div>
                  </div>
                </label>
                ...
                <label className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${exportMode === 'raw' ? 'border-black bg-zinc-50' : 'border-zinc-200 hover:border-zinc-300'}`}>
                  ...
                  <div>
                    <div className="font-bold text-zinc-900">RAW 파일만 복사</div>
                    <div className="text-xs text-zinc-500 mt-1 leading-relaxed">보정을 위해 용량이 큰 원본(RAW) 파일만 복사합니다.</div>
                  </div>
                </label>
                ...
                <label className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${exportMode === 'jpg' ? 'border-black bg-zinc-50' : 'border-zinc-200 hover:border-zinc-300'}`}>
                  ...
                  <div>
                    <div className="font-bold text-zinc-900">JPG 파일만 복사</div>
                    <div className="text-xs text-zinc-500 mt-1 leading-relaxed">가벼운 JPG 파일만 추출합니다.</div>
                  </div>
                </label>
              </div>
